...
import { User as UserIcon, Phone, TrendingUp, Loader2, ShieldCheck } from 'lucide-react';
...
export function OnboardingPage({ onComplete }: OnboardingPageProps) {
  ...
  // SMART KENYAN PHONE FORMATTER
  // Turns "0741..." into "254741..." behind the scenes for M-Pesa API
  const formatPhoneNumber = (number: string) => {
    let cleaned = number.replace(/[^\d+]/g, ''); // Strip spaces and dashes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. NICKNAME VALIDATION (Now allows spaces!)
    ...
    // Regex allows letters, numbers, and spaces
    ...
    // Checks if it is a valid 12-digit Kenyan number (2547... or 2541...)
    ...
    setIsLoading(true);
    try {
      // Send the beautifully formatted data to the backend
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 relative bg-zinc-950">
      <div className="w-full max-w-md space-y-8 text-center mt-12 sm:mt-0">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-zinc-400">Complete your profile to enter the market.</p>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-8 shadow-xl flex flex-col items-center">
          ...
          <div className="w-full p-4 mb-8 rounded-lg bg-zinc-950/50 border border-zinc-800 text-left flex gap-3">
            ...
            <p className="text-zinc-400 text-sm leading-relaxed">
              Your display name is public on the leaderboard. Your M-Pesa number is strictly private and used exclusively for secure deposits and withdrawals.
            </p>
          </div>
